...
void drawBody(){
     ...
     for(int i=1; i<body.size(); ++i){  //Body
          ...
          drawPoint(body[i], color(.8, .6, 0), cell-4);
          drawPoint(body[i], color(0, .6, 0), 3);
     }
     drawPoint(body[0], color(.8, .3, .0), cell);
     drawPoint(body[0], color(.4, .3, 0), cell-2);//
     if(dir==0 || dir==1){
...
point findFood(int cnt=0){
     ...
     if(cnt>100){
          for(int j=50; j>0; --j){
               for(int i=x; i<grid.size(); i+=j) if(!checkPoint(grid[i])) return grid[i];
               for(int i=0; i<x; i+=j) if(!checkPoint(grid[i])) return grid[i];
          }
     }
...
void init(){
     glClearColor(1.0, 1.0, 1.0, 0);//0.4,0.8,.4,0
     glClear(GL_COLOR_BUFFER_BIT);
